...
###   xcoincore.org and xcoin.org and compares them.
...
WORKINGDIR="/tmp/xcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://xcoincore.org"
HOST2="https://xcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="xcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'xcoin-',
...
if [ $? -ne 0 ]; then
   echo "xcoin.org failed to provide signature file, but xcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "xcoin.org and xcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Xcoin Core binary release signing key installed?"
   fi
