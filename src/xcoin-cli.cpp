...
static void SetupCliArgs()
{
    ...
    gArgs.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to xcoind)", false, OptionsCategory::OPTIONS);
    ...
    if (response.status == 0) {
        ...
        if (failedToGetAuthCookie) {
            throw std::runtime_error(strprintf(
                ...
                GetConfigFile(gArgs.GetArg("-conf", XCOIN_CONF_FILENAME)).string().c_str()));
        } else {
