...
ClientModel is used by the main application `xcoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/xcoin/xcoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a xcoin:... link from a browser).
...
* UI elements like XcoinAmountField, which inherit from QWidget.
* `xcoinstrings.cpp`: automatically generated
* `xcoinunits.(h/cpp)`: XCN / mXCN / etc handling
* `callback.h`
...
* don't change `local/xcoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
