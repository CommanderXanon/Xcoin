...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", XCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
...
#ifndef WIN32
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", XCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
#else
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/CommanderXanon/>";
    const std::string URL_WEBSITE = "<http://www.CommanderX.info>";
...
/** Sanity checks
 *  Ensure that Xcoin is running in a usable environment with all
...
bool AppInitMain()
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", XCOIN_CONF_FILENAME)).string());
